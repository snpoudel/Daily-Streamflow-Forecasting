#code for hbv foreccasting model
#that takes calibrated parameters, initial conditions and forecast inputs to forecast streamflow
#for this calibrate the model first, then re-run the calibrated model for the recent input data
#to prepare the initial conditions for the model and then forecast streamflow for the next 28 days
...
import math
# hbv function
def hbv_forecast(pars, storage, sim_vars, p, temp, date, latitude, routing):
    '''
    hbv function Input:
    - pars: parameter vector
    - storage: initial storage values [state_upres, state_lowres, state_snow, state_sliq, state_sma]
    - sim_vars: initial simulated variables [sim_snow, sim_swe, sim_melt, pr_eff, sim_et, sim_pexc, sim_sma, inflow_direct, inflow_base]
    ...
    sim_snow = np.repeat(sim_vars[0], len(p)) #simulated snow
    sim_swe = np.repeat(sim_vars[1], len(p)) #simulated snow water equivalent
    sim_melt = np.repeat(sim_vars[2], len(p)) #simulated snow melt
    pr_eff = np.repeat(sim_vars[3], len(p)) #effective precip (amount of liquid water available to enter soil matrix at a time step)

    sim_et = np.repeat(sim_vars[4], len(p)) #simulated actual evapotranspiration
    sim_pexc = np.repeat(sim_vars[5], len(p)) #simulated effective rainfall
    sim_sma = np.repeat(sim_vars[6], len(p)) #simulated soil moisuture storage accounting tank
    inflow_direct = np.repeat(sim_vars[7], len(p)) #direct flow
    inflow_base = np.repeat(sim_vars[8], len(p)) #base flow

    #initial storage values
    state_upres = storage[0]  #initial storage in upper zone/reservoir
    state_lowres = storage[1] #initial storage in lower zone/reservoir
    state_snow = storage[2] #initial state of snow storage
    state_sliq = storage[3] #initial state of liquid water on snowpack
    state_sma = storage[4] #initial state of soil moisture storage
