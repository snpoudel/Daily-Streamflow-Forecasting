...
arima_model = SARIMAX(y_train, order=best_order, seasonal_order=best_seasonal_order, exog=None).fit()
...
for test_year in range(2009, 2016):
    ...
    for month in range(1, 12):  # Loop through all 12 months
        ...
        try:
            ...
            results = SARIMAX(y_test_monthly, order=best_order, seasonal_order=best_seasonal_order, exog=None).fit(maxiter=500, disp=False)
